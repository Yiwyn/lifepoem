...
public void testOrderStatus() {
    ...
    assertEquals("待处理", status.getDescription());
    assertEquals(1, status.getCode());
    
    // 测试枚举的方法
    assertTrue(OrderStatus.COMPLETED.isFinalStatus());
    assertFalse(OrderStatus.PENDING.isFinalStatus());
    
    // 测试枚举的转换
    OrderStatus fromString = OrderStatus.valueOf("PROCESSING");
    assertEquals(OrderStatus.PROCESSING, fromString);
}

@Test
public void testEnumInSwitch() {
    String result = processOrder(OrderStatus.COMPLETED);
    assertEquals("处理已完成订单", result);
}
```
